...
/// Public site (keep in sync with `NEXT_PUBLIC_WEB_APP_URL` default in the frontend).
const PGSTUDIO_WEB: &str = "https://pgstudio-web.vercel.app";
const PGSTUDIO_CHANGELOG_URL: &str = "https://pgstudio-web.vercel.app/changelog";
...
fn about_metadata() -> AboutMetadata<'static> {
    AboutMetadata {
        ...
        website: Some(PGSTUDIO_WEB.to_string()),
...
pub fn install_native_menu<R: Runtime>(app: &mut App<R>) -> tauri::Result<()> {
    ...
    app.on_menu_event(move |app, event| {
        ...
        match id {
            ...
            "help_release_notes" => emit_open_url(app, PGSTUDIO_CHANGELOG_URL.to_string()),
            ...
            "help_website" => emit_open_url(app, PGSTUDIO_WEB.to_string()),
            "open_docs" => emit_open_url(app, PGSTUDIO_WEB.to_string()),
            "quit_app" => {
                app.exit(0);
            }
            _ => {}
