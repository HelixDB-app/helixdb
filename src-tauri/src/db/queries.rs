...
// ─── Lock inspector (`pg_locks` blocking graph + inventory) ───────────────────

const LOCK_INVENTORY_CAP: i64 = 500;

/// Wait-for edges and a capped lock inventory for the current database.
pub async fn get_lock_inspector(pool: &Pool) -> Result<super::types::LockInspectorData, String> {
    let client = pool.get().await.map_err(|e| e.to_string())?;
    let _ = client
    ...
    let wait_edges: Vec<super::types::LockWaitEdge> = wait_rows
        .iter()
        .map(|row| super::types::LockWaitEdge {
    ...
    let inventory: Vec<super::types::LockInventoryRow> = inv_rows
        .iter()
        .map(|row| super::types::LockInventoryRow {
            pid: row.get("pid"),
            locktype: row.get::<_, String>("locktype"),
    ...
    Ok(super::types::LockInspectorData {
        wait_edges,
        inventory,
        inventory_truncated,
    })
