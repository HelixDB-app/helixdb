use crate::auth_layer::AuthState;
use axum::body::Body;
...
/// HTTP application graph shared by the standalone binary and the Vercel serverless entrypoint.
pub fn build_router() -> Router {
    ...
    let jwks_url = env::var("DATA_PLANE_JWKS_URL")
        .ok()
        .filter(|s| !s.is_empty());
    let jwks = jwks_url.map(|u| Arc::new(crate::auth_layer::JwksCache::new(u)));
    ...
    let app = Router::new()
        ...
        .layer(from_fn(move |req: Request<Body>, next: Next| {
            ...
            async move { crate::auth_layer::auth_middleware_inner(a, req, next).await }
        }))
        .layer(cors_layer)
...
async fn connect(
    ...
    let (database_name, server_version) = crate::table_ops::get_server_info(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
...
async fn refresh_metadata(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let schemas = crate::table_ops::list_schemas(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(schemas))
}
...
async fn list_connection_databases(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let names = crate::catalog_meta::list_databases(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(names))
}
...
async fn create_database_http_handler(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    crate::catalog_meta::create_database(&pool, body.name.trim())
        .await
        .map_err(|e| (StatusCode::BAD_REQUEST, e))?;
    Ok(StatusCode::NO_CONTENT)
}
...
async fn drop_database_http_handler(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    crate::catalog_meta::drop_database(&pool, &db_name)
        .await
        .map_err(|e| (StatusCode::BAD_REQUEST, e))?;
    Ok(StatusCode::NO_CONTENT)
}
...
async fn list_schema_functions(
    ...
    let list = crate::catalog_meta::list_functions(&pool, &schema, pg_version)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(list))
}
...
async fn list_schema_types(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let list = crate::catalog_meta::list_types(&pool, &schema)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(list))
}
...
async fn list_tables(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let tables = crate::table_ops::list_tables(&pool, &schema)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(tables))
}
...
async fn create_table_http_handler(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let sql = crate::catalog_meta::create_table(
        &pool,
        &schema,
        body.table.trim(),
        &body.columns,
...
async fn table_search_handler(
    ...
    let result = crate::table_ops::search_table_data_multi(
        &pool,
        &schema,
        &table,
        &body.conditions,
...
async fn get_rows(
    ...
    let result = crate::table_ops::get_table_data(
        &pool,
        &schema,
        &table,
        page,
...
async fn execute_query_handler(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let result = crate::query_exec::execute_query(&pool, &body.sql)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(result))
}
...
async fn list_sessions(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let sessions = crate::extended::get_sessions(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(sessions))
}
...
async fn terminate_session(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let ok = crate::extended::terminate_backend(&pool, pid)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(ok))
}
...
async fn cancel_session(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let ok = crate::extended::cancel_backend(&pool, pid)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(ok))
}
...
async fn get_database_topology(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let data = crate::extended::get_all_user_schemas_topology(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(data))
}
...
async fn get_topology(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let data = crate::extended::get_schema_topology(&pool, &schema)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(data))
}
...
async fn get_table_columns(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let cols = crate::extended::get_columns(&pool, &schema, &table)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(cols))
}
...
async fn get_table_details_handler(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let details = crate::extended::get_table_details(&pool, &schema, &table)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(details))
}
...
async fn list_connection_event_triggers(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let list = crate::catalog_meta::list_event_triggers(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(list))
}
...
async fn get_function_definition_handler(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let def = crate::catalog_meta::get_function_definition(&pool, &schema, &function_name, &q.arguments)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(def))
}
...
async fn get_type_definition_handler(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    let detail = crate::catalog_meta::get_type_definition(&pool, &schema, &type_name)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;
    Ok(Json(detail))
}
...
async fn create_enum_handler(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    crate::catalog_meta::create_enum(&pool, &schema, &body.name, &body.values)
        .await
        .map_err(|e| (StatusCode::BAD_REQUEST, e))?;
    Ok(StatusCode::NO_CONTENT)
}
...
async fn patch_enum_values_handler(
    ...
    let pool = state
        ...
        .get_pool(&connection_id)
        .map_err(|_| (StatusCode::NOT_FOUND, "unknown connection".to_string()))?;
    crate::catalog_meta::alter_enum_values(
        &pool,
        &schema,
        &type_name,
        &body.renames,
